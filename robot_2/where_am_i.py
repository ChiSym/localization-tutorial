...
class World(genjax.PythonicPytree):
    ...
    @jax.jit
    def ray_distance(self, ray_start: jnp.ndarray, ray_dir: jnp.ndarray, max_dist: float) -> float:
    ...
    @jax.jit 
    def check_movement(self, start_pos: jnp.ndarray, end_pos: jnp.ndarray, 
                      collision_radius: float = WALL_COLLISION_THRESHOLD) -> Tuple[bool, jnp.ndarray]:
...
class RobotCapabilities(genjax.PythonicPytree):
    ...
    def try_move(self, world: World, current_pos: jnp.ndarray, 
                desired_pos: jnp.ndarray, key: PRNGKey) -> jnp.ndarray:
...
@jax.jit
def get_sensor_readings(world: World, robot: RobotCapabilities,
    ...
    readings = jax.vmap(lambda a: world.ray_distance(
        ray_start=pose.p,
        ray_dir=jnp.array([
            jnp.cos(pose.hd + a),
            jnp.sin(pose.hd + a)
...
@jax.jit
def execute_control(world: World, robot: RobotCapabilities,
