...
#
# In mathematical parlance, a stochastic function $g$ from $X$ to $X'$ would correspond to a probability kernel $k_g \colon X \dashrightarrow X'$, meaning a family of probability distributions $k_{g,x}$ on $X'$ indexed by $x$ in $X$, with each $k_{g,x}$ describing the distribution of results of calling $g$ on the input $x$.  We would be seeking the density $k_{g,x}(x')$ with which the sample $x' \sim k_{g,x}$ occurs.  Pursuing this approach requires knowlege of all execution histories of $g$ that might have produced any output, and then performing a sum over them.  For some small finite situations this may be fine, but this general problem of computing marginalizations is computationally impossible.
#
# The marginalization question is especially forced upon us when trying to compose stochastic functions.  Given a second stochastic function $g'$ from $X'$ to $X''$, corresponding to a probability kernel $k_{g'} \colon X' \dashrightarrow X''$, the composite $g' \circ g$ from $X$ to $X''$ should correspond to the probability kernel $k_{g' \circ g} \colon X \dashrightarrow X''$ whose samples $x'' \sim k_{g' \circ g,x}$ are defined by "sample $x' \sim g_x$, then sample $x'' \sim g'_{x'}$, then return $x''$".  Computing the density $k_{g' \circ g,x}(x'')$, even if one can compute $k_{g,x}(x')$ and $k_{g',x'}(x'')$ for any given $x,x',x''$, would require summing or integrating over all possible intermediate values $x'$ (which manifests an "execution history" of $g' \circ g$) that could have intervened in producing $x''$ given $x$.
#
...
#
# The only thing a program can reasonably be expected to know is the score/weight/density of its arriving at its return value *via the particular stochastic computation path* that got it there, and the approach of Gen is to report this number.  The corresponding mathematical picture imagines GFs as factored into *distributions over choice maps*, whose score/weight/density is computable, together with *deterministic functions on these data* that produce the return value from them.  That is, the data of GF $g$ from $X$ to $X'$ has an auxiliary space $U_g$ containing all of the choice map information, a probability kernel $k_g \colon X \dashrightarrow U_g$ with computable density embodying the stochastic execution history, and an old-fashioned (deterministic) function $\text{ret}_g \colon X \times U_g \to X'$ embodying extraction of the return value from the particular stochastic execution.
#
# In the toy example `g` above, choice map consists of `flip` so the space $U_g$ is binary; the deterministic computation $\text{ret}_g$ amounts to the `return` statement; and the score/weight/density is `p` or `1.0-p`, regardless of whether the inputs are equal.
#
# Tractable compositionality holds in this formulation.  If another GF $g'$ from $X'$ to $X''$ has data $U_{g'}$, $k_{g'}$, and and $\text{ret}_{g'}$, the the composite GF $g' \circ g$ from $X$ to $X''$ has the following data.  The auxiliary space is $U_{g' \circ g} := U_g \times U_{g'}$.  The kernel $k_{g' \circ g}$ is defined by "sample $u \sim k_{g,x}$, then compute $x' = \text{ret}_g(x,u)$, then sample $u' \sim k_{g',x'}$, then return $(u,u')$", and its density is computed via $k_{g' \circ g, x}(u,u') := k_{g,x}(u) \cdot k_{g',\text{ret}_g(x,u)}(u')$.  The return value function is $\text{ret}_{g' \circ g}(x,(u,u')) := \text{ret}_{g'}(\text{ret}_g(x,u),u')$.  As one composes more GFs, the auxiliary space accumulates more factors $U$, as the "execution history" consists of longer and longer records.
#
# One may still be concerned with the distribution on return values in this picture.  This information is still embodied in the aggregate of the stochastic executions that lead to any return value, together with their weights.  (Consider that this is true even in the toy example.)  In a sense, when we kick the can of marginalization down the road, we can proceed without difficulty.
#
...
#
# The following question soon arises: supposing we have on hand some properly weighted sampler $\~Q$ for $P$, where the values $z$ have type $X$, as well as some other target distribution $P'$ on values $z'$ of type $X'$, how might we concoct a properly weighted sampler $\~Q'$ for $P'$?  We might start by writing a generative function from $X$ to $X'$ to transform values $z$ into values $z'$.  So sampling $(w,z) \sim \~Q$ and applying the transform to $z$ determines a proposal $Q'$ on $X'$.  It clearly would behoove us to engineer the transform to bring $Q'$ as near as possible to $P'$.  But to close the remaining gap, for what construction of an *incremental weight* $\~w$ is it true that, setting $w' := w \cdot \~w$, the resulting distribution $(w',z') \sim \~Q'$ is properly weighted for $P'$?  The concept of *SMCP<sup>3</sup>* (which generalizes so-called *SMC samplers* under composition) provides an answer to the incremental weight question, whose shape we now describe.
#
...
#
# In this notebook we will only apply SMCP<sup>3</sup> to generative function transformations upon a single space $X = X'$ with a single target distribution $P = P'$.  We will try to engineer the transformation so that it *improves sample quality*, that is, so that the resulting new proposal distribution $Q'$ might better approximate $P$ than $Q$ did.  Moreover, in our case, the bijection $\~g$ will amount to a permutation of some tuple components, so the Jacobian factor will be $J = 1$.  Thus SMCP<sup>3</sup> simplifies to take the following shape:
